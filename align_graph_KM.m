function [G1,perm_mtx] = align_graph_KM(G,H,M,fig,core_flag);
...
% updated August 11, 2020 by Katie to allow for core comparison only
% relative to the given mask M (in case H has extra fixed points outside of
% M that you want to ignore)
% updated August 15, 2020 by Katie clarifying the core misalignment message
...
for i=1:size(all_perms,1)
    ...
    if core_flag
        Gp_core = Gcore(p,p);
        Mcore=M.*mask;
        if isempty(find(Gp_core.*Mcore - Hcore.*Mcore))
            c(i)=1;
            %            fprintf('cores align!\n')
        end;
    end;
...
if core_flag && ~isempty(find(c==1))
    ...
    if core_flag
        fprintf('G could not be aligned to match the core motifs of H that lie within the mask M \n');
    end
    dmin = min(d);
    idx = find(d == dmin);
    core_str = [];
