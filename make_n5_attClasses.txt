...
% Jan 17, 2021
%
% organizes graphs with dynamic attractors into structural families 
% with similar dynamics (same core motif, visual similarity of the 
% attractor, and identical or near-identical nullchamber sequence)
%
% ends with: 
% save('n5_attClasses','graphClass','attClass','classRule',...
%     'classLabel','sAcell2','X0cell2','sAcom','sAopt','sAmiss',...
%     'gCcom','gCopt','gCmiss','doubled_graphs','attPerms');
%
% note: the sAcell2 saved here is augmented from the one 
%       in n5_digraphs AND has already implemented the 
%       permutations in attPerms
%
% updated Jan 9, 2021 to fix remaining secondary attractor issues
% and introduce graphClass for all the non-core attClasses (1-37)
%
% updated Jan 14, 2021 to introduce new permutations for some n=5 cores
% updated Jan 17 to fix some misclassified graphs
...
classRule{1} = [];
graphClass{1} = attClass{1};

% 3-cycle + node 4 bumps (251 graphs)
classLabel{2} = '3-cycle + node 4 bumps';
...
classRule{2} = [];
graphClass{2} = attClass{2};

% 3-cycle + synchronized node 4 & 5 bumps (36 + 31 = 67 graphs)
% rule (a): 4<->5 double edge or neither, symmetry-preserving
classLabel{3} = '3-cycle + synchronized node 4 & 5 bumps';
...
% 3-cycle + symmetry-broken node 4 & 5 bumps (60 graphs)
classLabel{4} = '3-cycle + symmetry-broken node 4 & 5 bumps';
...
classRule{4} = [];
graphClass{3} = [attClass{3} attClass{4}];

% 3-cycle + sequential node 4 & 5 bumps (225 graphs)
classLabel{5} = '3-cycle + sequential node 4 & 5 bumps';
...
attClass{11} = [71  74  87  89  91  96  103  110  120  126  127 ...
    ...
    10153];
classRule{11} = 'b';
graphClass{7} = attClass{11};

% 4-cycu + bump, no symmetry breaking v1 (8 graphs)
% rule (c): 5->2,3 both or neither, preserving 2-3 symmetry
classLabel{12} = '4-cycu + bump, no symmetry breaking v1';
attClass{12} = [72  77  121  138  100  107  141  151 10151];
classRule{12} = 'c';

% 4-cycu + bump, slight symmetry breaking (4 graphs)
classLabel{13} = '4-cycu + bump, slight symmetry breaking';
attClass{13} = [84  102  140  147];
classRule{13} = [];
graphClass{8} = [attClass{12} attClass{13}];

% 4-cycu + bump, no symmetry breaking v2 (8 graphs)
% rule (c): 5->2,3 both or neither, preserving 2-3 symmetry
classLabel{14} = '4-cycu + bump, no symmetry breaking v2';
attClass{14} = [123  128  129  150 106  109  111  118];
classRule{14} = 'c';

% 4-cycu + bump, medium symmetry breaking (4 graphs)
classLabel{15} = '4-cycu + bump, medium symmetry breaking';
attClass{15} = [80   92   94  114];
classRule{15} = [];

% 4-cycu + bump, large symmetry breaking (4 graphs)
% note: 10321 is a perm of graph 321 (a 4-ufd class)
classLabel{16} = '4-cycu + bump, large symmetry breaking';
attClass{16} = [132  137  152  10321 10152]; 
classRule{16} = [];
graphClass{9} = [attClass{14} attClass{15} attClass{16}];
...
classLabel{17} = 'missing 4-cycu attractor';
...
classLabel{18} = '4-cycu + big bump, no symmetry breaking';
...
classLabel{19} = '4-cycu + big bump, symmetry breaking';
attClass{19} = [93 113 136 10322];
classRule{19} = 'd';
graphClass{10} = [attClass{17} attClass{18} attClass{19}];
...
classRule{20} = 'e';
graphClass{11} = attClass{20};

% 4-ufd + small bump v1 (16 graphs)
% rule (c): 5->2,3 both or neither, preserving 2-3 symmetry
% rule (f): 2->5 XOR 3->5, symmetry-breaking
% (ordering reflects the four subclasses)
classLabel{21} = '4-ufd + small bump v1';
...
classLabel{22} = '4-ufd + small bump v2';
...
attClass{35} = [983   984   985   986   988   989   990   991 ...
    ...
    1047  1052 11052];
classRule{35} = [];
graphClass{16} = attClass{35};

% pure fusion 3-cycle v2, with 4->5 (8 graphs)
classLabel{36} = 'pure fusion 3-cycle v2, with 4->5';
attClass{36} = [987  1000  1001  1009  1015  1019  1034  1048];
classRule{36} = [];
graphClass{17} = attClass{36};

% fusion 3-cycle + node 5 bump (16 graphs)
% note: graph 318 was "missing" and is now permuted to align as 10318
classLabel{37} = 'fusion 3-cycle + node 5 bump';
attClass{37} = [992  1006  1016  1017  1020  1025  1027  1028 ...
    1036 1041  1045  1049  1050  1051  1053  10318 11053];
classRule{37} = [];
graphClass{18} = attClass{37};

% ** core motifs start, first for fusion attractors **

% fusion 3-cycle w/ 2-clique (n=5 core)
classLabel{38} = 'fusion 3-cycle w/ 2-clique';
attClass{38} = [10];
classRule{38} = [];

% fusion 4-cycle (n=5 core)
classLabel{39} = 'fusion 4-cycle';
attClass{39} = [7];
classRule{39} = [];

% fusion 4-cycu (n=5 core)
classLabel{40} = 'fusion 4-cycu';
attClass{40} = [8];
classRule{40} = [];

% fusion 4-ufd (n=5 core)
classLabel{41} = 'fusion 4-ufd';
attClass{41} = [9];
classRule{41} = [];
...
classLabel{49} = 'outer neuron on 3-cycle + variations';
attClass{49} = [19:24];
classRule{49} = 'g';

% cyclic union: pt, 2-clique, 2-clique + variations (embedded 4-cycu)
classLabel{50} = 'cyclic union: pt, 2-clique, 2-clique + variations';
attClass{50} = [25:30]; % (same nullchamber sequence)
classRule{50} = [];

% double envelope attractor 
classLabel{51} = 'extended 4-udf: double envelope attractor';
attClass{51} = [31];
classRule{51} = [];

% envelope attractor (same nullchamber sequence)
classLabel{52} = 'extended 4-ufd: envelope attractor';
attClass{52} = [32:36];
classRule{52} = [];

% double envelope attractor (dying 4-ufd)
classLabel{53} = 'dying 4-ufd: double envelope attractor';
attClass{53} = [37 38];
classRule{53} = [];

% weird attractor 
classLabel{54} = 'weird attractor';
attClass{54} = [39];
classRule{54} = [];


% group some core classes together
graphClass{19} = [attClass{47} attClass{52} attClass{53} attClass{44}];
...
% Find common graphs for each class and save _________________________

% NOTE: we are not yet checking for identical copies of graphs!
% so this may overcount... e.g. 908 & 10980 are the same graph wrt
% labeling and all, as are 982 & 10982.
